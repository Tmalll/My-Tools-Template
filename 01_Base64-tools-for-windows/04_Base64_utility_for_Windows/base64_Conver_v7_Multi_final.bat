...
if /I "%EXT%"==".b64" (
    ...
    call :wait_all "%TMPDIR%" ".dec.ok" %PARTS%
    ...
    call :wait_all "%TMPDIR%" ".b64.ok" %PARTS%
...
:: -----------------------
:: 子程序：等待所有标记就绪
:: 用法：call :wait_all <dir> <suffix> <parts>
:: 例如：call :wait_all "%TMPDIR%" ".dec.ok" 10
:wait_all
set "WA_DIR=%~1"
set "WA_SUF=%~2"
set "WA_CNT=%~3"
...
:wa_loop
set /a done=0
for /L %%j in (0,1,%WA_CNT%-1) do (
    if exist "%WA_DIR%\part%%j%WA_SUF%" set /a done+=1
)
if %done% lss %WA_CNT% (
    timeout /t 1 >nul
    goto wa_loop
