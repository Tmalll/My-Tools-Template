...
echo ==========================
echo File Split / Merge Tool (异步版)
echo 拖放文件即可分割或合并
echo 注意: 路径中不能有 "^!" "^`" "^'" "^"" 等符号...
echo ==========================
echo.
...
powershell -NoProfile -Command ^
  ...
  "$pad=%PART_PAD%;" ^
  "$fs=[IO.File]::Open($path,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::None);" ^
  ...
  "    $read=$fs.ReadAsync($buf,0,$toRead).GetAwaiter().GetResult();" ^
  "    if($read -le 0){break};" ^
  "    [void]($out.WriteAsync($buf,0,$read).GetAwaiter().GetResult());" ^
  "    $remaining-=$read;" ^
  "  }" ^
  "  $out.Close();" ^
...
powershell -NoProfile -Command ^
  ...
  "$idx=0;" ^
  "foreach($p in $parts){" ^
  "  $fs=[IO.File]::Open($p.FullName,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::None);" ^
  "  while(($read=$fs.ReadAsync($buf,0,$bufSize).GetAwaiter().GetResult()) -gt 0){" ^
  "    [void]($out.WriteAsync($buf,0,$read).GetAwaiter().GetResult());" ^
  "  }" ^
  "  $fs.Close();" ^
  "  $idx++;" ^
