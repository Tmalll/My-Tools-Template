...
if /I "%EXT%"==".b64" (
    ...
    set /a done=0
    for /L %%i in (0,1,%PARTS%-1) do (
        if exist "%TMPDIR%\part%%i.dec.ok" set /a done+=1
    )
    if not !done!==%PARTS% (
        timeout /t 1 >nul
        goto wait_decode
    )
    :: 合并块 —— 去掉 .b64 的原始名，并使用延迟展开
    set "OUTRAW=%~dpn1"
    copy /b NUL "!OUTRAW!" >nul
    for /L %%i in (0,1,%PARTS%-1) do (
        if exist "%TMPDIR%\part%%i.dec" type "%TMPDIR%\part%%i.dec" >> "!OUTRAW!"
    )
    echo 完成: !OUTRAW!
) else (
    echo 编码中...
    ...
    set /a done=0
    for /L %%i in (0,1,%PARTS%-1) do (
        if exist "%TMPDIR%\part%%i.b64.ok" set /a done+=1
    )
    if not !done!==%PARTS% (
        timeout /t 1 >nul
        goto wait_encode
